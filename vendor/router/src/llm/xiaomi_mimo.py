...
PRICE_INPUT_CACHE_HIT  = Decimal("0.00000001")  # $0.01 / 1M (cache hit)
PRICE_INPUT_CACHE_MISS = Decimal("0.00000010")  # $0.1  / 1M (cache miss)
PRICE_OUTPUT           = Decimal("0.00000030")  # $0.3  / 1M (output)


def summarize_response_stats(response_json: dict, decimals: bool = True) -> dict:
    try:
        cache_hit_tokens = utils.default_for_none(response_json["usage"]["prompt_tokens_details"], {}).get("cached_tokens", 0)
        cache_miss_tokens = response_json["usage"]["prompt_tokens"] - cache_hit_tokens
