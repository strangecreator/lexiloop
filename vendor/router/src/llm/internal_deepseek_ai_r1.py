...
PRICE_INPUT_CACHE_HIT  = Decimal("0.000003")     # $3.00 / 1M (cache hit)
PRICE_INPUT_CACHE_MISS = Decimal("0.000003")     # $3.00 / 1M (cache miss)
PRICE_OUTPUT           = Decimal("0.000007")     # $7.00 / 1M (output)


def summarize_response_stats(response_json: dict, decimals: bool = True) -> dict:
    try:
        cache_hit_tokens = response_json["response"]["usage"]["cached_tokens"]
        cache_miss_tokens = response_json["response"]["usage"]["prompt_tokens"] - response_json["response"]["usage"]["cached_tokens"]
