...
PRICE_INPUT_CACHE_HIT  = Decimal("0.000000175")   # $0.175 / 1M (cache hit)
PRICE_INPUT_CACHE_MISS = Decimal("0.000001750")   # $1.750 / 1M (cache miss)
PRICE_OUTPUT           = Decimal("0.000014000")   # $14    / 1M (output)


def summarize_response_stats(response_json: dict, decimals: bool = True) -> dict:
    try:
        cache_hit_tokens = response_json["response"]["usage"]["prompt_tokens_details"]["cached_tokens"]
        cache_miss_tokens = response_json["response"]["usage"]["prompt_tokens"] - cache_hit_tokens
